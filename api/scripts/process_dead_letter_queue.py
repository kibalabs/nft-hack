...
async def main():
    ...
    dlWorkQueue = SqsMessageQueue(region='eu-west-1', accessKeyId=os.environ['AWS_KEY'], accessKeySecret=os.environ['AWS_SECRET'], queueUrl='https://sqs.eu-west-1.amazonaws.com/097520841056/mdtp-work-queue-dl')
    messageQueueProcessor = MessageQueueProcessor(queue=dlWorkQueue, messageProcessor=processor, slackClient=slackClient)
    ...
    try:
        while True:
            hasProcessedWork = await messageQueueProcessor.execute_batch(batchSize=1, longPollSeconds=1, shouldProcessInParallel=True)
