...
class MdtpSaver(Saver):
    ...
    async def create_grid_item(self, tokenId: int, network: str, title: str, description: Optional[str], imageUrl: str, resizableImageUrl: Optional[str], ownerId: str, lastUpdateBlockNumber: int) -> GridItem:
        ...
        gridItemId = await self._execute(query=GridItemsTable.insert(), values={
            ...
            GridItemsTable.c.lastUpdateBlockNumber.key: lastUpdateBlockNumber,
        })
        return GridItem(gridItemId=gridItemId, createdDate=createdDate, updatedDate=updatedDate, network=network, tokenId=tokenId, title=title, description=description, imageUrl=imageUrl, resizableImageUrl=resizableImageUrl, ownerId=ownerId, lastUpdateBlockNumber=lastUpdateBlockNumber)
    ...
    async def update_grid_item(self, gridItemId: int, title: Optional[str] = None, description: Optional[str] = _EMPTY_STRING, imageUrl: Optional[str] = None, resizableImageUrl: Optional[str] = _EMPTY_STRING, ownerId: Optional[str] = None, lastUpdateBlockNumber: Optional[int] = None) -> None:
        ...
        if lastUpdateBlockNumber is not None:
            values[GridItemsTable.c.lastUpdateBlockNumber.key] = lastUpdateBlockNumber
        if len(values) > 0:
