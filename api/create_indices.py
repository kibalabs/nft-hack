...
def get_spiral_index(index: int, size: int) -> int:
    # odd layers are top and left
    # even layers are bottom and right
    layer = math.ceil(math.sqrt(index)) - 1
    print(size, index, layer)
    isOdd = size % 2 == 0
    if index == 0:
        previousOddCount = math.floor((size - 1) / 2)
        print(previousOddCount)
        previousEvenCount = math.floor((size + 1) / 2)
        print(previousEvenCount)
        # 0 is always at the middle for odds and same position as previous odd for evens
        numbersAbove = size * previousOddCount
        numbersLeft = previousOddCount
        return numbersAbove + numbersLeft
    minIndex = 0
    maxIndex = (size * size) - 1
    layerMin = (layer - 1) * (layer - 1)
    layerMax = (layer * layer) - 1
    print(size, index, layerMin, layerMax)
    return index

class Tests(unittest.TestCase):

    def _test_size1(self):
        # 0    0
        self.assertEqual(get_spiral_index(index=0, size=1), 0)
