...
sleep 3
docker exec ${cid} bash -c "export PGPASSWORD=${DB_PASSWORD}; exec psql -h ${DB_HOST} -U ${DB_USERNAME} -c 'CREATE DATABASE ${DB_NAME}'"
docker exec ${cid} bash -c "export PGPASSWORD=${DB_PASSWORD}; exec psql -h ${DB_HOST} -U ${DB_USERNAME} -d ${DB_NAME} -f /tmp/dblocal.sql"
docker logs -f $cid
